...
setkey(gxpDataDT, POC) # warning - POC column renames to node in next step
...
ggplot2::ggplot(plotDT, aes(x = hms, y = totDaily/1000, colour = season)) +
  ...
  labs(x = "Date", y = "Total daily MWh per half-hour", 
       caption = paste0(myCaption, "\nN GXPs: ", uniqueN(gxpDataDT$MXLOCATION))) +
...
gxpWinter2015DT <- gxpDataDT[year == 2015 & 
  ...
  t <- gxpWinter2015DT[is.na(kWh), .(MXLOCATION, Time_Period, rDateTime, kWh)]
  head(t, 10)
  # check
  summary(t)
  # yep, all NA
  ...
  table(gxpSelectWinter2015DT$MXLOCATION, gxpSelectWinter2015DT$regionName, useNA = "always")
  # one of them seems to have double the observations. Why?
  table(gxpSelectWinter2015DT$MXLOCATION, gxpSelectWinter2015DT$NWK_Code, useNA = "always")
