...
XX compare with earlier results

```{r compareMethods}
# merge with total load from above
setkey(totalLoadProfileDT, hms, season)
setkey(plotDT, hms, season)
dt <- plotDT[totalLoadProfileDT]

dt[, diff := totalResMWh2015/ipfTotalMWh]

ggplot2::ggplot(dt, aes(x = hms)) + 
  geom_line(aes(y = ipfTotalMWh), colour = "black") +
  geom_line(aes(y = totalResMWh2015), colour = "grey") +
  facet_wrap(. ~ season) +
