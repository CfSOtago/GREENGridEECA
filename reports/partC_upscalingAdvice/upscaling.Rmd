...
As an example, Table \@ref(tab:ggHeatSource) shows the 'main method of heating' for the GREEN Grid sample and Figure \@ref(fig:ggHeatSource) compares these with the results from the BRANZ Housing Conditions Survey 2015 [@white_branz_2017]. As we can see the GREEn Grid sample has a higher heat pump rate than the HCS 2015 estimate of 40% (+/-6.1%) heat pump penetration for owner occupied homes (the most similar to the GREEN Grid sample) and 25% (+/-10.8%) for rentals. On the other hand, 25% of the GREEN Grid sample used enclosed woodburners as a main method of heating compared to HCS 2015 estimates of 39% and 23% for owener-occupied and rental dwellings respectively.

>Insert footnote re HCS confidence intervals of +/-6.1% for owner-occupier homes and +/-10.8% for rentals at the 95% level (Table 1, [@white_branz_2017]. This means that if we repeated the HCS 100 times we would expect that in 95 of them the % of owner occupier dwellings with heat pumps would be between 33.9% and 46.1% and the figure for rentals would be 14.2% to 35.8%. Clearly the small sample size of the HCS 2015 leads to substantial uncertainty in the estimates of heat pump prevalance (and all other estimates) in these groups. For a more detailed discussion of this problem see [@anderson_ensuring_2020].

On the other hand relatively few GREEN Grid households used portable electric or gas heaters =compared to the HCS 2015 sample.
...
ggplot2::ggplot(combined_dt, aes(x = Appliance , y = pc, fill = type)) +
  ...
  coord_flip() +
  labs(x = "Appliance", y = "%",
       caption = "Non-comparable appliances excluded, totals will not sum to 100%\nError bars = 95% CI for HCS 2015")


```

Further, Table \@ref(tab:compareheatSource) compares the 'Fuel type used to heat dwelling' for Census 2013 and the 'main method of heating' for the GREEN Grid sample. To do this we have recoded the GREEN Grid data summarised in Table \@ref(tab:ggHeatSource) to match the Census but note the virtual absence of gas and coal in the GREEN Grid sample.
...
t <- rbind(pc_ct, # total census data
           crt_pc[source  %like% "Taranaki" | source %like% "Hawke"], # regions
           pc_ht) # GG

kableExtra::kable(t, digits = 2, 
                  caption = "Comparing main heating fuel/method (% of dwellings)") %>%
  kable_styling()
```

> XX can we have more than 1 fuel source in the census data? 
