...
In this method, rather than assume that the base (GREEN Grid) sample represents all New Zealand consunmotion profiles we proportionately adjust the profiles using some other data source. 

### Example: Adjusting for different available heat sources

Table \ref(tab:ggHeatSource) showed that there were differences between the GREEN Grid sample and the overall household population in terms of main heat source used. We might expect main space heat source to make a difference to the load profile of the households. In principle we can use this information to proportionately adjust the estimates by calculating overall kWh profiles for each main heat type in the GREEN Grid sample. We would then multiplying these different profiles by the number of each household type we would expect in the population calculated by combining Census household counts with HCS 2015 estimates of heat source prevalence (see Table \ref(tab:ggHeatSource)).

Unfotunately, as Table \ref(tab:ggHeatSourceN) shows, the GREEN Grid sample has too few cases in all but the heat pump category to make this feasible. 
...
# keep comparables for clarity
# dt <- dt[Appliance == "Heat pump" |
#         Appliance == "Enclosed wood burner" |
#         Appliance == "Portable electric heaters" |
#         Appliance == "Portable gas heaters"]

kableExtra::kable(dt, digits = 1,
                  caption = "N dwellings in GREEN Grid sample by main heat source") %>%
  kable_styling()
```
...
Unfortunately, as Table \@ref(tab:ggTenure) shows, over 90% of GREEN Grid dwellings were owner-occupied and so the approach of calculating different consiumptionprofiles for each tenure type is not feasible. 
...
Nevertheless, if we assume non systematgic differences between the tenure types we can continue with the example. To do this we first calculate the overall mean electricity consumption profiles for heat pump circuits based on the GREEN Grid data (Figure \@ref(fig:prepHpProfiles)).
 ...
 * `r prHouseholdsOwn*nzHhPop2015` owner occupied homes of which `r nOwnHPmid` have heat pumps
 * `r prHouseholdsRent*nzHhPop2015` rental homes of which `r nRentHPmid` have heat pumps
 * `r prRest*nzHhPop2015` 'other tenure' homes of which `r nOtherHPmid` have heat pumps
