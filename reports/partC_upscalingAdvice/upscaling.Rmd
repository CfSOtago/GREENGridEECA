...
To produce regional or national estimates we first prepare the household level consumption data to provide mean energy consumption (kWh) per half-hour for each season in 2015. We use kWh to enable comparison with the GXP data used in Part B.
...
winterMax <- max(profileDT[season == "Winter"]$meankWh)
```

Figure \@ref(fig:prepHHProfiles) shows the mean kWh per half hour for each season that we will use as the basis for the re-weighting process. As we can see we estimate the maximum mean kWh per half-hour in winter to be ~ `r winterMax` kWh giving a rough estimate of maximum mean residential kWh in winter as `r round((winterMax * nzHhPop2015)/1000000,2)` GWh.
