...
Figure \@ref(fig:prepHpProfiles) shows the results of these intial sample-based profile calculation and we estimate that the maximum mean kWh per half-hour consumed by heat pumps in winter in 2015 was ~ `r winterHpMax` kWh. As before, if we multiply this by the number of households estimated by the [2015 NZ Statistics household count projection](http://archive.stats.govt.nz/browse_for_stats/population/estimates_and_projections/DwellingHouseholdEstimates_HOTPJun17qtr.aspx) (`r tidyNum(nzHhPop2015)`) then we get a rough estimate of maximum mean residential kWh in winter of `r round((winterHpMax * nzHhPop2015)/1000000,2)` GWh.
 ...
 * `r tidyNum(nOwnHPmid)` owner occupied homes of which `r tidyNum(nOwnHPmid)` have heat pumps
 * `r tidyNum(nRentHPmid)` rental homes of which `r tidyNum(nRentHPmid)` have heat pumps
 * `r tidyNum(nOtherHPmid)` 'other tenure' homes of which `r tidyNum(nOtherHPmid)` have heat pumps
...
Figure \@ref(fig:compareHpGxp2015) shows these values as a % of the total network electricity export by season for all regions in 2015. We can see that the estimated % contribution of residential dwellings to peak time consumption is small - < 2% in summer but up to just over 6% in winter. In the absence of any other data sources we are unable to comment on the plausibility of these estimates.
...
In this example we use the IPF method to reweight the GREEN Grid sample so that it fits to the 2013 NZ census distributions of the variables listed above at the national level. 
...
As \@ref(tab:ipfWeightsNational) shows, the household level weights required range from `r min(ipfNationalWDT$ipfWeight)` for relatively common combinations of the constraint variables to `r max(ipfNationalWDT$ipfWeight)` for less common combinations.

Figure \@ref(fig:ipfTotalLoadNational) shows the results of this re-weighting process
...
Figure \@ref(fig:compareMethods) compares the simple multiplicative results from Section \@ref(totalLoad2015) and the IPF-derived results. As we can see the IPF method produces lower overall consumption estimates and this appears to be especially the case during the day in summer. This is likely to be cuased by the upweighting of the PV-equipped dwellings although we note that the IPF results are generally lower at all times of day and in all seasons.
 ...
 * -ve values side-effect of inflating the 4 PV owning households
 * reduced peak demand not due to PV but to more realistic re-weighting of the GREEN Grid smaple

#### Regional estimates

In this example we use the IPF method to reweight the GREEN Grid sample so that it fits to the 2013 NZ census distributions of the variables listed above at the regional levels. 
...
As \@ref(tab:ipfWeightsRegional) shows, the household level weights required range from `r min(ipfRegionalWDT$ipfWeight)` for relatively common combinations of the constraint variables to `r max(ipfRegionalWDT$ipfWeight)` for less common combinations.
...
Figure \@ref(fig:ipfTotalLoadRegional) shows the results of this re-weighting process
