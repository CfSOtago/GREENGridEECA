...
summaryPSDT$season <- factor(summaryPSDT$season, levels = c("Spring","Summer",
                                                    "Autumn", "Winter"))#This puts the seasons in the right order in the plot
```
Figure \@ref(fig:wattageContributionPeakSamplePlot) shows the mean contribution of each identifiable load for the sample for one of the identified peak half hours ( 2015-06-28 at 18:00:00). The results show that Hot Water contributes the most, followed by Heat Pumps and Lighting. The plot also shows that much power was drawn from circuits other than the identified ones (summarized in “Others”). Figure \@ref(fig:wattageContributionPeakSamplePlotPercentage) shows the associated percentage contribution. Since the chosen half-hour is on the 2015-06-28 at 18:00:00 (winter) Heat Pump and Lighting demand are clearly contributing to peak demand, however, Hot Water is the circuit contributing the most.
...
```{r wattageContributionPeakSamplePlot, fig.cap="Average appliance contribution to peak demand on the 2015-06-28 at 18:00:00 (one of the top 20 half hours in the sample)"}
...
#----------------------------------
#PartA2 to supply EECA data table

consloadEECADT <- copy(powerDT)#Utilise adjusted raw data

consloadEECADT <- consloadEECADT[season  == "Winter" &
             year == "2015" &
             obsHalfHour >= hms::as.hms("17:00:00") &
             obsHalfHour <= hms::as.hms("21:00:00")] #This is our definition of peak demand for this analysis regarding the conservation load factor

consloadEECADT <- consloadEECADT[, averageSeasonW := mean(meanPowerW, na.rm = TRUE), keyby =.(season, eecaCircuit, year, linkID)]#This gives us the average power at peak for each household and season and circuit


consloadEECADT <- consloadEECADT[, .(averageSeasonW =averageSeasonW), keyby =.(season, eecaCircuit, year, linkID)]#Long to short dt
consloadEECADT<-  unique(consloadEECADT)# Removing duplicates XX_BE VERY CAREFUL WITH THIS_XX

consloadEECADT <- subset(consloadEECADT, eecaCircuit == "Heat Pump or Heating"| 
                       eecaCircuit == "Hot water"|
                       eecaCircuit == "Lighting"|
                       eecaCircuit == "Oven"|
                       eecaCircuit == "XX_Other")

consloadEECADT <- consloadEECADT[, averagecontri := ConsLoadFactorSupplyDT$averageContri]
consloadEECADT <- consloadEECADT[, consLoadFac := averageSeasonW / averagecontri ]#Now do the caclculation based on the Jacoby report. A completely different picture now!

consloadEECADT <- dcast(consloadEECADT,
           season + year + linkID  ~ eecaCircuit, 
           fun = mean, # won't do anything as we should just have 1 value?
           value.var = "consLoadFac")


consloadEECADT <- setnames(consloadEECADT, old=c("Heat Pump or Heating","Hot water", "Lighting", "Oven", "XX_Other"),
                        new=c("Heat Pump", "Hot Water", "Lighting", "Oven", "Others"))

consloadEECADT <- data.table::as.data.table(gather(consloadEECADT, eecaLabelContri, consLoadFac, `Heat Pump`:`Others`, factor_key = TRUE))#This brings our data into a nice form for the plot

write_csv(consloadEECADT, "/Users/carsten.dortans/Dropbox/GreenGrid_EECA/data/csv_full_data_tables/Extracted load factors for all houses for 2015 in winter between 17_21_00.csv")
...
#_XX It turns out the provided definition to calculate the Cons load factor was wrong. We do not need the meant total energy but the average demand at peak... I copied our results from the first definition of peak demand (this is the average wattage of maximum half hours at peak demand at 17-21:00.)




ConsLoadFactorSupplyDT <- subset(ConsLoadFactorSupplyDT, season=="Winter")#This is our copied result

consloadDT <- subset(consloadDT, eecaCircuit == "Heat Pump or Heating"| 
                       eecaCircuit == "Hot water"|
                       eecaCircuit == "Lighting"|
                       eecaCircuit == "Oven"|
                       eecaCircuit == "XX_Other")

consloadDT <- consloadDT[, averagecontri := ConsLoadFactorSupplyDT$averageContri]
consloadDT <- consloadDT[, consLoadFac2 := meanAverageSeasonW / averagecontri ]#Now do the caclculation based on the Jacoby report. A completely different picture now!
...
consloadCastDT <- dcast(consloadDT,
           ...
           fun = mean, # won't do anything as we should just have 1 value?
           value.var = "consLoadFac2")


consloadCastDT <- setnames(consloadCastDT, old=c("Heat Pump or Heating","Hot water", "Lighting", "Oven", "XX_Other"),
                        new=c("Heat Pump", "Hot Water", "Lighting", "Oven", "Others"))

consloadCastDT <- data.table::as.data.table(gather(consloadCastDT, eecaLabelContri, consLoadFac2, `Heat Pump`:`Others`, factor_key = TRUE))#This brings our data into a nice form for the plot

```

The Load Factor (LF) is defined as the ratio of average demand (W) in any given half-hour? to peak demand (W). The LF varies between 0 and 1 and provides a measure of how much the load contributes to peak. If the LF for a particular load is 1 then the load is flat. If it is less than one it shows that the load disproportionately contributes to the peak. In this section we define peak demand to be the maximum demand between 17:00-21:00 in winter (see first definition of peak demand).
