...
powerDT[, meanEnergyWh := meanPowerW/2]
...
# test table
t <- powerDT[year == 2015, .(sumkWh = sum(meanEnergyWh/1000, na.rm = TRUE),
...
keepVars <- c("season", "ba_peak","meanTotalkWh","meankW","sdkW","nHouseholds")

new <- c("Season", "Peak", "Mean total kWh", "Mean kW", "s.d. (kW)", "N households")

table_t <- t[, ..keepVars]

setnames(table_t, keepVars, new)

kableExtra::kable(table_t, digits = 2, caption = "Descriptive statistics for power data") %>%
  kable_styling()
```
...
```{r plotkWh, fig.cap="Mean total kWh per period by season (2015)"}

# plot sums at household level
plotDT <- powerDT[year == 2015, .(sumkWh = sum(meanEnergyWh/1000, na.rm = TRUE),
                             meankW = mean(meanPowerW/1000, na.rm = TRUE),
                             sdkW = sd(meanPowerW/1000, na.rm = TRUE),
                 nDays = uniqueN(date),
...
```{r plotkW, fig.cap="Mean kW per half hour by period and season (2015)"}
...
analysisDT <- powerDT[, totalEnergyWh := sum(meanEnergyWh, na.rm = TRUE), 
...
analysisShortDT <- analysisShortDT[, pc_HW := `Hot water`/Total]
analysisShortDT <- analysisShortDT[, pc_HP := `Heat Pump or Heating`/Total]
analysisShortDT <- analysisShortDT[, pc_LI := `Lighting`/Total]
analysisShortDT <- analysisShortDT[, pc_OV := `Oven`/Total]
analysisShortDT <- analysisShortDT[, pc_Other := Other/Total]
...
t <- dt[year == 2015, .(meankWh = mean(totalEnergykWh, na.rm = TRUE),
            mediankWh = median(totalEnergykWh, na.rm = TRUE),
            sdkWh = sd(totalEnergykWh, na.rm = TRUE),
            nHouseholds = uniqueN(linkID)), keyby = .(year, eecaCircuit)]

kableExtra::kable(t, caption = "Descriptive statistics for 2015 (annual kWh)") %>%
  kable_styling()

message("# Test for comparison with EECA Energy End Use Database")

# total mean kWh across households
s <- sum(t$meankWh)
# lighting as a % of household mean
pc <- 100*(t[eecaCircuit == "Lighting", meankWh]/s)
message("Mean lighting kWh as a % of mean total kWh - actually sum of means so...")
pc
# If household consumption is 32% of all elec consumption (as per EECA) then can we
# just multiply this value by 32% to see how close we get to the EECA 4% figure for 
# residential lighitng a a % of all electricity?
message("30% of that value - EECA Elec end use database suggests residential consumtion is 32% of total")
pc * 0.32

message(" -> which is slightly less than 4% so it suggests our households use less than EECA estimate or EECA estimate is too high")
...
```{r annualConsPlotHW, fig.cap="Distribution of seasonal energy consumption for Hot Water for all houses for 2015", warning=FALSE}
...
```{r annualConsPlotHP, fig.cap="Distribution of seasonal energy consumption for Heat Pumps for all houses for 2015", warning=FALSE}
...
```{r annualConsPlotL, fig.cap="Distribution of seasonal energy consumption for Lighting for all houses for 2015", warning=FALSE}
...
```{r annualConsPlotOven, fig.cap="Distribution of seasonal energy consumption for Ovens for all houses for 2015", warning=FALSE}
...
```{r annualConsPlotOther, fig.cap="Distribution of seasonal energy consumption for Other for all houses for 2015", warning=FALSE}
...
```{r consLoadPlot, fig.cap="Mean Conservation Load Factor by circuit for peak demand (17:00-21:00) in winter 2015"}
