...
write_csv(resAllSeasonsDT, "/Users/carsten.dortans/Dropbox/GreenGrid_EECA/data/csv_full_data_tables/Extracted average demand by dwelling and circuit at 'peak'.csv")
...
write_csv(copiedtable, "/Users/carsten.dortans/Dropbox/GreenGrid_EECA/data/csv_full_data_tables/Extracted peak load half-hour circuits for Hawke's Bay.csv")
...
write_csv(copiedtable, "/Users/carsten.dortans/Dropbox/GreenGrid_EECA/data/csv_full_data_tables/Extracted peak load half-hour circuits for Taranaki.csv")
...
MergedDT <- merge(AverageDemandDT, AveragePeakDemandDT, by = "linkID", all = TRUE, allow.cartesian = TRUE )
...
The Load Factor (LF) is defined as the ratio of average demand (W) across all half hours to peak demand (W). The LF varies between 0 and 1 and provides a measure of how much the load contributes to peak. If the LF for a particular load is 1 then the load is flat. If it is less than one it shows that the load disproportionately contributes to the peak. In this section we define peak demand to be the maximum demand between 17:00-21:00 in winter (see first definition of peak demand).

Figure \@ref(fig:consLoadPlot) shows the Load Factor (LF) for each identifiable load averaged over all houses in the sample. Based on this definition power was calculated as an average over all households. This calculation builds the average seasonal demand by circuit, linkID, and season and then averages this over the houses. In a second step, we utilise the identified maximum demand used for the first definition of peak demand to estimate the LF. Results show that the LF for Lighting (0.53) is the highest followed by Heat Pump (0.47) and Hot Water (0.25). Please note that these numbers represent  averages over all households, dates, and times for 2015.

The full data table containing LFs (if available) for each house is saved as "Extracted load factors for all houses for 2015 in winter between 17_21_00.csv" and supplied with the report.
...
myPlot <- ggplot2::ggplot(consloadCastDT, aes(x= eecaLabelContri, fill = eecaLabelContri)) +
  geom_bar(aes(y = consLoadFac2), stat = "identity")+
  theme(text = element_text(family = "Cambria"),
        axis.text.y = element_blank(),
        axis.text.x = element_text(colour = "black"),
        legend.position = "none",
    ...
    geom_text(aes(y = consLoadFac2,
             label = sprintf("%0.2f",consLoadFac2)),
            position = position_stack(vjust = .5))+
