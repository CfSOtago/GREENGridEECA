...
```{r partBdataProcessing, warning=FALSE}
...
* Sample winter evenings 17:00 - 21:00. In this case we:
...
To do this last part we classify the circuit labels according to the load types mentioned above (see Table \@ref(tab:findMaxHalfHours) in the Data Annex).

## Winter (June - August)  evenings 17:00 – 21:00

###Average load
In this section we select the peak-period half hours for all households for the winter evening period and then calculate the avergage demand for each dwelling over the period of peak demand.

Table \@ref(tab:averageAtPeak) shows the winter average demand (W) at 'peak' for each dwelling and circuit in 2015. The complete data table across all houses is saved as “Extracted average demand by dwelling and circuit at 'peak'.csv” and is supplied with the report.
...
dataBucketDT <- GREENGridEECA::labelEECACircuits(selectDT) # use the function to split the circuit string for easier string matching

#Calculating average demand over the period for each dwelling and circuit

dataBucketDT <- dataBucketDT[, avgPeakDemandW := NA]
dataBucketDT <- dataBucketDT[, .(avgPeakDemandW = mean(meanPowerW, na.rm = TRUE)), keyby =.(linkID, eecaCircuit)]


kableExtra::kable(head(dataBucketDT, 10), digits = 3,
                  caption = "Example data: Extracted average demand by dwelling and circuit at 'peak'")  %>% # test result
  kable_styling()


WriteDT <- data.table::fwrite(resAllSeasonsDT, paste0(here::here(), "/data/output/Extracted average demand by dwelling and circuit at 'peak'.csv"))
...
ContriW1DT <- data.table::as.data.table(gather(ContriW1DT, eecaCircuit, avgPeakDemandW, `Heat Pump`:`Others`, factor_key = TRUE))#This brings our data into a nice form for the plot
...
```{r wattageContributionPeak1Plot1, fig.cap="Average contribution at 'peak' (17:00-21:00) by circuit in winter 2015"}
...
```
```{r wattageContributionPeak1PlotPercent1, fig.cap="Average percentage contribution at 'peak' (17:00-21:00) by circuit in winter 2015"}
...
We have then used this data to calculate the mean contribution of each identifiable load to peak demand (time period 17:00-21:00) in the sample for winter in 2015. This is shown in Figure \@ref(fig:wattageContributionPeak1Plot1) and Figure \@ref(fig:wattageContributionPeak1PlotPercent1) (as percentage contribution). On average, Heat Pumps contributed the most to peak demand, followed by Hot Water, Lighting, and Oven. Not further specified load contributed 32%. 
...
###Maximum load
In this section we select the peak-period half hours for all households for the winter evening period and then locate the half-hour which had the maximum total load for that dwelling.
...
Table \@ref(tab:winterPeaktestTable) shows the winter maximum load half-hours for each dwelling and year in winter during the peak periods defined above. In this data extract, NaN indicates that no circuit data was obtained for this dwelling in this specific time period. We also calculate the proportionate contribution of each identifiable load to the total load. The complete data table across all houses is saved as “Extracted winter peak load half-hour circuits.csv” and is supplied with the report.
...
WriteDT <- data.table::fwrite(resAllSeasonsDT, paste0(here::here(), "/data/output/Extracted winter peak load half-hour circuits.csv"))
...
We have then used this data to calculate the mean contribution of each identifiable load to peak demand (time period 17:00-21:00) in the sample and this is shown in Figure \@ref(fig:wattageContributionPeak1Plot) and Figure \@ref(fig:wattageContributionPeak1PlotPercent) (as percentage contribution). Figure \@ref(fig:wattageContributionPeak1Plot) shows the seasonal variation in actual power demand especially for heat pumps and lighting but also shows the relative constancy of hot water. ‘Others’ almost certainly contains some forms of heating as it is much lower in summer. It should be noted that these mean values will mask considerable day to day and inter/intra-household variation On the other hand, and as a consequence the percentage contribution of hot water slightly increases in summer whereas the contribution of heat pumps decreases over summer. Hot water is identified as the appliance that is contributing the most to peak demand in this case.


```{r wattageContributionPeak1Plot, fig.cap="Average contribution to daily peak demand (17:00-21:00) per season in 2015"}
...
```
```{r wattageContributionPeak1PlotPercent, fig.cap="Average percentage contribution to peak demand (17:00-21:00) for each season in 2015" }
...
In this section we focus on the contribution of these identifiable loads to regional peak demand.  Most of the households in our data set were in the Hawke’s Bay and Taranaki region, so we focus on these regions by separating the sample into the Hawke’s Bay and Taranaki sub-samples.

22 houses are based in Taranaki and 20 houses in Hawke's Bay. Within the house sample in Taranaki, 31% of the houses have heat pumps installed, 45% hot water cylinders, and 27% other heat sources. In contrast, the Hawke's Bay sub-sample comprises of 90% heat pumps, 90% hot water cylinder, and 36% other heat sources.

e.g. see https://cfsotago.github.io/GREENGridData/householdAttributeProcessingReport_v1.0.html#51_main_heat_source 

To determine the timing of the matching regional peaks, data on regional demand for the Hawke’s Bay and Taranaki Grid Exit Point (GXP) was obtained from the Electricity Authority and for each GXP, the timestamps of the 100 half hours with the highest demand were identified. We then extracted the power demand of each dwelling in each of the two regions during these 100 half hours for analysis.
...
GXPtaranakiDT <- data.table::as.data.table(
 readr::read_csv("/Users/carsten.dortans/Desktop/R Profiles/github/CFSOtago/GREENGridEECA/data/output/taranakiGxpTop100DT.csv",
                       col_types = cols(
...
GXPHawkesDT <- data.table::as.data.table(
 readr::read_csv("/Users/carsten.dortans/Desktop/R Profiles/github/CFSOtago/GREENGridEECA/data/output/hawkesBayGxpTop100DT.csv",
                       col_types = cols(
                       region = col_character(),
                       regionName = col_character(),
                       rDateTime = col_datetime(format = ""),
...
extractHawkesDT <- data.table::as.data.table(
 readr::read_csv("/Users/carsten.dortans/Desktop/R Profiles/github/CFSOtago/GREENGridEECA/data/output/extractHawkes.csv",
                       col_types = cols(
...
extractTaranakiDT <- data.table::as.data.table(
 readr::read_csv("/Users/carsten.dortans/Desktop/R Profiles/github/CFSOtago/GREENGridEECA/data/output/extractTaranaki.csv",
                       col_types = cols(
                       linkID = col_character(),
                       nObs = col_double(),
                       r_dateTime_nz = col_datetime(format = ""),
...
Table \@ref(tab:RegionalAnalysisHawkesContribution) presents example data of the extracted peak half-hours by circuit for 2015 for the Hawke’s Bay dwellings. In this example data they are matching to a number of regional peak half hours in June 2015. The complete data table is saved as “Extracted peak load half-hour circuits for Hawke’s Bay.csv” and is supplied with the report.
...
The average contribution of each identifiable load to the Hawke’s Bay regional peak demand  (100 half hours with the highest demand at Hawke’s Bay GXP) for the sample is shown in Figure \@ref(fig:wattageContributionHawkesPlot). Hot Water and Heat Pumps contribute a mean of 682 Watts and 672 Watts respectively but ‘Others’ (which probably includes heating) contributes 792. Heat Pumps and Hot Water contributed almost equally to regional network peak demand in Hawke’s Bay (27% and 28%), followed by Lighting (10%) and Oven (3%). However, ‘Other’ demand that was not further specified contributed 32%. This is presented in Figure \@ref(fig:wattageContributionHawkesPlotPercentage). it should be noted that the inherent bias in the sample, and the very small sample size in each region means that these % results are almost certainly not robust.

```{r wattageContributionHawkesPlot, fig.cap="Average contribution of each identifiable load in the sample to the Hawke’s bay regional peak (100 half hours with the highest demand at Hawkes Bay GXP) for 2015"}
...
```{r wattageContributionTaranakiPlot, fig.cap="Average contribution of each identifiable load to peak demand in Taranaki in 2015"}
...
In this section the sum of half-hourly demand over all dwellings in the the GREENGrid sample is used to define peak demand. We define the peak as the 20 half hours with the highest total half-hourly demand summed across all dwellings, which we refer to as the sample coincident peak. This is in contrast to section 4.1 where peak demand was predefined as the time between 17:00 and 21:00 in winter and section 4.2 where peak demand was predefined as the 100 half hours with the largest demand based on regional GXP data.

Table \@ref(tab:sampleCoincidentPeak) identifies the half hours of highest overall demand in 2015 using the sum of the half-hourly mean power values (summeanPowerW). As we can see the number of dwellings contributing to this grand sum is between 31 and 33 and the table is sorted in descending order of the mean total power across all contributing dwellings. Clearly we would expect summed values with more contributing dwellings to be higher and so we control for ‘missing dwellings’ by sorting by and using the mean total power (last column) as our indicator. The complete data table is saved as "Extracted winter peak load half-hours in descending order for 2015" and is supplied with the report.
                
```{r sampleCoincidentPeak}

sampleDT <- labelEECACircuits(powerDT)

sampleDT <- sampleDT[, sumOfmeanPowerW := NA]

sampleShortDT <- sampleDT[, .(sumOfmeanPowerW = sum(meanPowerW, na.rm = TRUE), 
...
WriteDT <- data.table::fwrite(resAllSeasonsDT, paste0(here::here(), "/data/output/Extracted winter peak load half-hours in descending order for 2015.csv"))
...
summaryPSDT$season <- factor(summaryPSDT$season, levels = c("Spring","Summer",
                                                    "Autumn", "Winter"))#This puts the seasons in the right order in the plot
```
Figure \@ref(fig:wattageContributionPeakSamplePlot) shows the mean contribution of each identifiable load for the sample for the top 20 identified peak half hours cross the sample. The results show that on avergae Heat Pumps contribute the most, followed by Hot Water and Lighting. The plot also shows that much power was drawn from circuits other than the identified ones (summarized in “Others”).

Again, these results should be viewed with some caution due to the non-representative nature of the GREEN Grid sample. While they represent the sample co-incident peak contributions, we cannot assume that this is representative of all New Zealand dwellings.
...
ContriWSDT <- ContriWSDT[, totalW := sum(averageContri), keyby =.(season, r_dateTime_nz)]
...
```{r wattageContributionPeakSamplePlotAll, fig.cap="Average appliance contribution to peak demand for the top 20 half hours in the sample)"}


ContriWSDT[, dateChar := as.factor(r_dateTime_nz)]
ContriWSDT[, dateChar := format(r_dateTime_nz, format='%Y-%m-%d %H:%M')]

myPlot <- ggplot2::ggplot(ContriWSDT, aes(x= dateChar, fill = eecaLabelContri)) +
  geom_bar(aes(y = averageContri), stat = "identity")+
  coord_flip()+
...
This section presents annual and seasonal energy consumption (in kWh) for each identifiable load. This analysis considers the year 2015. Figure \@ref(fig:annualplot) shows the annual total energy consumption (in kWh) for the identifiable loads in a box and whisker plot to show the central distribution and also outliers. Hot Water consumes the most energy per year with a pronounced variation between houses. Lighting, Heat Pump, and Oven follow. Not further specified circuits are summarised in “Others” but this group clearly drives large inter-household variation.
...
```{r annualplot, fig.cap="Box and whisker plot of annual energy consumption (per house) of each identifiable load for 2015", warning=FALSE}
...
For "Others", there is a lot of seasonal variation in seasonal energy consumption as shown in Figure \@ref(fig:annualConsplot5). As this circuit contains appliances not further specified it is difficult to identify the appliance causing this energy consumption, particularly in summer. Therefore, further research is necessary.


```{r annualConsplot5, fig.cap="Distribution of seasonal energy consumption for Others for all houses for 2015", warning=FALSE}

myPlot <- ggplot2::ggplot(dt[eecaCircuit =="Others"], aes(x=totalEnergyWh/1000, colour=season)) +
  geom_density() +
  labs(x = "Seasonal energy (kWh)") +
  scale_x_continuous( limits = c(0,500))+
     geom_vline(aes(xintercept=mean(totalEnergyWh/1000)),
...
```{r annualConsplotwinter, fig.cap="Distribution of seasonal energy consumption for Hot Water for all houses for 2015", warning=FALSE}

myPlot <- ggplot2::ggplot(dt2[season =="Winter"], aes(x=totalEnergyWh/1000, colour=Circuit)) +
  geom_density() +
...
```
Furthermore, Figure \@ref(fig:annualConsplotwinter) shows the variation in circuits for winter 2015. On average, Hot Water used the most energy, followed by Heat Pumps, Lighting, and Ovens. Not further specifies load is indicated as 'Others'.
...
AverageDemandDT <- GREENGridEECA::labelEECACircuits(selectDT) # use the function to split the circuit string for easier string matching
AverageDemandDT <- AverageDemandDT[, avgDemandW := NA]
AverageDemandDT <- AverageDemandDT[, .(avgDemandW = mean(meanPowerW, na.rm = TRUE)), keyby =.(linkID, eecaCircuit)]
...
AveragePeakDemandDT <- GREENGridEECA::labelEECACircuits(selectDT) # use the function to split the circuit string for easier string matching

AveragePeakDemandDT <- AveragePeakDemandDT[, avgPeakDemandW := NA]
AveragePeakDemandDT <- AveragePeakDemandDT[, .(avgPeakDemandW = mean(meanPowerW, na.rm = TRUE)), keyby =.(linkID, eecaCircuit)]
...
WriteDT <- data.table::fwrite(MergedDT, paste0(here::here(), "/data/output/Extracted load factor by household and circuit.csv"))




kableExtra::kable(head(MergedDT, 10), digits = 3,
...
Merged2DT <- dcast(Merged2DT, 
           linkID ~ eecaCircuit, 
           fun = mean, # won't do anything as we should just have 1 value?
           value.var = "LoadFactor") # take 1 household and turn data round


Merged2DT <- setnames(Merged2DT, old=c("Heat Pump or Heating","Hot water", "Lighting", "Oven", "XX_Other"),
                        new=c("Heat Pump", "Hot Water", "Lighting", "Oven", "Others"))
...
#Summarising the variation in demand
#We need to dcast it back into the original form
dt <- data.table::as.data.table(gather(Merged2DT, eecaCircuit, LoadFactor, `Heat Pump`:`Others`, factor_key = TRUE))


dt$eecaCircuit <- factor(dt$eecaCircuit, levels = c("Hot Water","Heat Pump",
                                                    "Lighting", "Oven", "Others"))#This puts the seasons in the right order in the plo
...
```{r LoadFactorAverage}
dt3 <- copy(dt)
dt3 <- dt3[, .(meanLF = mean(LoadFactor, na.rm= TRUE)), keyby =. (eecaCircuit)]

```

The Load Factor (LF) is defined as the ratio of average demand (W) across all half hours to peak demand (W). The LF provides a measure of how much the load contributes to peak. If the LF for a particular load is 1 then the load is flat. Values greater 1 indicate that the average demand outside the peak period is higher than the average demand inside it. A LF less than 1 shows that the load disproportionately contributes to the peak. In this section we define peak demand to be the average demand between 17:00-21:00 in winter. Table \@ref(tab:LoadNew) shows the first rows of our calculation.

The full data table containing LFs (if available) for each house is saved as "Extracted load factor by household and circuit.csv" and supplied with the report.

Figure \@ref(fig:consLoadPlot) shows the Load Factor (LF) for each identifiable load averaged over all houses in the sample. Results show that the LF for Hot Water (1.19) is the highest followed by Heat Pump (0.47), Oven (0.38), and Lighting (0.34). Please note that these numbers represent  averages over all households, dates, and times for 2015. Furthermore, the average LF over all houses is skewed by some houses with a LF > 6 as Figure \@ref(fig:histoLF) shows.

```{r histoLF, fig.cap="Load Factor distribution by circuit for peak demand (17:00-21:00) in winter 2015"}


  
    
myPlot <- ggplot2::ggplot(dt, aes(x= eecaCircuit, y=LoadFactor)) +
  geom_count(aes(y = LoadFactor), stat = "identity")+
  theme(text = element_text(family = "Cambria"),
        axis.text.y = element_text(colour = "black"),
        axis.text.x = element_text(colour = "black"),
        legend.position = "none",
...
```{r consLoadPlot, fig.cap="Load Factor by circuit for peak demand (17:00-21:00) in winter 2015"}


myPlot <- ggplot2::ggplot(dt3, aes(x= eecaCircuit, fill = eecaCircuit)) +
  geom_bar(aes(y = meanLF), stat = "identity")+
  theme(text = element_text(family = "Cambria"),
        axis.text.y = element_blank(),
        axis.text.x = element_text(colour = "black"),
        legend.position = "none",
    ...
    geom_text(aes(y = meanLF,
             label = sprintf("%0.2f",meanLF)),
            position = position_stack(vjust = .5))+
