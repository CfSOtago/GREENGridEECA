...
analysisDT <- analysisDT[, totalEnergyWh := sum(meanEnergyWh), keyby = .(linkID, season, eecaCircuit, year)]
analysisDT <- analysisDT[, .(totalEnergyWh = totalEnergyWh), by = .(linkID, season, eecaCircuit, year)]#For some reson by does not remove duplicates without crashing....
analysisDT <- unique(analysisDT)#Removing duplicates

#Calculating the contribution to annual energy consumption and turning off e-notation

options(scipen = 999)

analysisDT <- dcast(analysisDT, 
           linkID + year + season ~ eecaCircuit, 
           fun = mean, # won't do anything as we should just have 1 value?
           value.var = "totalEnergyWh") # take 1 household and turn data round

analysisDT <- analysisDT[, contribution_HW := `Hot water`/Total]
analysisDT <- analysisDT[, contribution_HP := `Heat Pump or Heating`/Total]
analysisDT <- analysisDT[, contribution_LI := `Lighting`/Total]
analysisDT <- analysisDT[, contribution_OV := `Oven`/Total]
analysisDT <- analysisDT[, contribution_XX := 1-((`Hot water` + `Heat Pump or Heating` + `Lighting` + `Oven`)/Total)]
