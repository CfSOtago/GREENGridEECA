...
```
Figure \@ref(fig:2015contributionPlot) shows the percentual contribution to peak demand for 2015 for each season and circuit. Please note, that the stacked bar chart does not add up to 100% as we averaged the circuit contributions over all households.
...
In addition, Figure \@ref(fig:wattageContributionPeak1Plot) verifies what Figure \@ref(fig:2015contributionPlot) already indicated. Hot water, on average, contributes the most peak demand.
...
Data on regional demand for the Hawke's Bay and Taranaki Grid Exit Point (GXP) was obtained from the Electriicty Authority and the top 100 demand half hours (i.e. the date time of the top 100 half hours) were extracted from this data. Based on this list of date times we extract the associated power values from the previously subsetted power data table for Hawke's Bay and Taranaki where the GXP data matches the date time in the power data table.
...
Table \@ref(tab:RegionalAnalysisHawkesContribution) presents example data of the successfully extracted peak load half-hours by circuit for 2015. Peak in the following is defined as the period of time where the to 100 GXP date times occur.
...
```{r RegionalAnalysisHawkesContribution2}
...
Figure \@ref(fig:2015contributionPlot) shows the percentual contribution to peak demand as an average over the date times in the GXP data by circuits. Heat Pumps and Hot Water contributed almost equally to peak demand in Hawke's Bay (27% and 30%), followed by Lighitng (12%) and Oven (3%). Other demand that was not further specified contributed 38%. Please note that these percentages to not add up to 100% as we averaged over the dwellings and date times.


```{r 2015contributionPlotHawkesGXP, fig.cap="Percentual contribution to peak demand for top 100 GXP half hours in Hawke's Bay in 2015"}
...
In terms of average wattage contribution of the top 100 GXP date times in Hawke's Bay we identify a similar pattern. Hot Water and Heat Pumps contribute 682 Watts and 672 Watts respectively. 


```{r wattageContributionHawkesPlot, fig.cap="Wattage contribution to peak demand for top 100 GXP half hours in Hawke's Bay for 2015 "}
...
Table \@ref(tab:RegionalAnalysisTaranakiContribution) presents example data of extracted peak load half-hours by circuit for 2015 in Taranaki.
...
```{r RegionalAnalysisTaranakiContribution2}
...
Figure \@ref(fig:2015contributionPlotTaranakiGXP) shows the percentual contribution to peak demand as an average over the date times in the GXP data by circuits. Lighting contributes the most to peak demand with 24%, followed by Heat Pumps (22%), Hot Water (11%), and Oven (7%). Not further specified loads contribute 35% to peak demand in Taranaki. Please note that these percentages to not add up to 100% as we averaged over the dwellings and date times.

```{r 2015contributionPlotTaranakiGXP, fig.cap="Percentual contribution to peak demand for top 100 GXP half hours for Taranaki in 2015"}
...
```{r wattageContributionTaranakiPlot, fig.cap="Wattage contribution to peak demand for top 100 GXP half hours in Taranaki for 2015 "}
...
The sample coincident peak utilizes the GREENGrid sample to define peak demand. This is in contrast to the previous section where peak demand was predefined as the time between 17:00 and 21:00 in winter.

Table \@ref(tab:sampleCoincidentPeak) identifies the half hours of peak demand in 2015 for the total sample load. Results are presented in a descending order. Please note that the aim of this data table is to identify the date and time of overall sample peak demand. To do that, we have to consider the number of observations for each half hour. The displayed average power in Watts is, therefore, not the total for the half-hour but a division of the sum of wattage per half-hour and the number of observations.
...
Figure \@ref(fig:2015contributionPlotSample) shows one of the identified top 20 peak half hours in 2015. The data was averaged over the households and is presented as a percentual contribution. Since the chosen half-hour of peak demand is on the 2015-06-28 at 18:00:00 (winter) heat pump and lighting demand are clearly contributing to peak demand. Furthermore, how water is the circuit contributing the most.
...
```{r wattageContributionPeakSamplePlot, fig.cap="Wattage contribution to peak demand on the 2015-06-28 at 18:00:00 (one of the top 20)  "}

myPlot <- ggplot2::ggplot(ContriWSDT[r_dateTime_nz == "2015-06-28 18:00:00"], aes(x= eecaLabelContri, fill = eecaLabelContri)) +
  geom_bar(aes(y = averageContri), stat = "identity")+
  theme(text = element_text(family = "Cambria"),
        axis.text.y = element_blank(),
        axis.text.x = element_text(colour = "black"),
...
This section presents annual energy consumption for a variety of circuits and the variability between dwellings. This analysis takes the year 2015 into account.
...
Figure \@ref(fig:annualConsplot1) shows the variation in annual energy consumption for Hot Water in 2015 by season and considers all dwellings. Please note that Figure \@ref(fig:annualConsplot1) shows energy in kWh and not power demand in Watts. Results show that most energy consumption of Hot Water is in winter, followed by spring and autumn.


```{r annualConsplot1, fig.cap="Annual kWh for Hot Water considering each dwelling by season in 2015", warning=FALSE}
...
```
In contrast, Figure \@ref(fig:annualConsplot2) presents the annual energy consumption for Heat Pumps and considers each dwelling for each season. As expected, most energy consumption is in winter. In summer, total energy consumption is mostly 0 (appliance is turned off). Only a few observatons registered energy consumption in summer (cooling?). 

```{r annualConsplot2, fig.cap="Annual kWh for Heat Pumps considering each dwelling by season in 2015", warning=FALSE}
...
```
Energy consumption of Lighting is particularly apparent spring, autumn, and winter as presneted in Figure \@ref(fig:annualConsplot3). Less energy consumption is identified in summer.


```{r annualConsplot3, fig.cap="Annual kWh for Lighting considering each dwelling by season in 2015", warning=FALSE}
...
For ovens, there is less seasonal variation in total energy consumption as shown in Figure \@ref(fig:annualConsplot3). Furthermore, total energy consumption in summer is less than in the other seasons.

```{r annualConsplot4, fig.cap="Annual kWh for Ovens considering each dwelling by season in 2015", warning=FALSE}
...
Figure \@ref(fig:consLoadPlot) shows the Conservation Load Factor (CLF) for winter in 2015. The period of peak demand was set to 17:00-21:00 in winter. Based on this definition power was calculated as an average over all households. Results show that the CLF for Heat Pumps (0.56) is the highest followed by Lighting (0.45) and Oven (0.41). Please note that these numbers are averages over all households, dates, and times for 2015.
