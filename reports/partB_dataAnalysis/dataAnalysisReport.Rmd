...
proDT <- powerDT[, sumMeanPoW := ifelse(season == 'Winter' & obsHalfHour >= '17:00:00' & obsHalfHour <= '21:00:00', sum(meanPowerW), 0), keyby = .(circuit, obsHalfHour, date)]


  
proDT <- proDT[!(proDT$sumMeanPoW==0)]
proDT <- proDT[, .(sumMeanPoW = sumMeanPoW), 
                    keyby = .(circuit, obsHalfHour, date)]
proDT <- proDT[, maxPoW := max(sumMeanPoW), keyby =.(circuit)]

proDT %>% distinct()

#proDT <- proDT[, .(maxPoW = maxPoW), keyby =.(circuit)]
