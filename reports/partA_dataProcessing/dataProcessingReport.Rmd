...
Table \@ref(tab:extractHalfHour_Lighting) shows the mean power (mean of the half-hourly mean values)
for all observations extracted by dwelling ID. Note that in some cases circuits may contain other appliances. Analysis should therefore proceed with caution.

```{r extractHalfHour_Lighting}
...
hhLightingDT <- hhPowerDataDT[circuit %like% filter]

kableExtra::kable(makeTable(hhLightingDT), 
                  caption = paste0("Summary statistics for `", filter, "` extract"))
```
...
Table \@ref(tab:extractHalfHour_HotWater) shows the mean power (mean of the half-hourly mean values)
for all observations extracted by dwelling ID. Note that in some cases circuits may contain other appliances. Analysis should therefore proceed with caution.

```{r extractHalfHour_HotWater}

hhHotWaterDT <- hhPowerDataDT[circuit %in% filter1 |
                                circuit %in% filter2]

kableExtra::kable(makeTable(hhHotWaterDT), 
                  caption = paste0("Summary statistics for `", filter, "` extract"))
```
