 ...
 * not all dwellings contained the appliance circuits of analytic interest;
 ...
 * a tile plot showing the data avilability and mean power for the extracted circuits over the entire data collection;
...
filter <- "Lighting"
dt <- hhPowerDataDT[circuitLabel %like% filter] # extraction
```
...
makeTilePlot <- function(dt){
  ...
  p <- ggplot2::ggplot(plotDT, aes(x = r_dateTimeHalfHour, 
                                   ...
                                   alpha = nObs)) +
    geom_tile() +
    scale_alpha_continuous(name = "N dwellings") +
